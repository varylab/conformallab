...
public class FibonacciClusteringG2 {
	...
	public static void main(String[] args) throws Exception {
		...
		Thread CR_job = new Thread(new Runnable() {
			@Override
			public void run() {
				FibonacciClusteringG2.calculate(
		...
		Formatter out = new Formatter(FibonacciClusteringG2.filename + ".r");
		out.format(FibonacciClusteringG2.rTemplate,
			P.normSqr(),
			join(CR_rPnorm),
			join(CR_maxLengths),
			join(CF_rPnorm),
		...
		for (int i = 0; i < count; i++) {
			...
			out.format("#  Po[%2$d]: %1$s\n", commentOut(CR_rPo[i]), i);
			out.format("#  P[%2$d] : %1$s\n", commentOut(CR_rP[i]), i);
			out.format("# |P[%2$d]|: %1$s\n", CR_rP[i].normSqr(), i);
			out.format("# |L[%2$d]|: %1$s\n", CR_maxLengths[i], i);
			out.format("# |a[%2$d]|: %1$s\n", CR_minAngle[i], i);
			HalfedgeIO.writeOBJ(CR_rS[i], a, FibonacciClusteringG2.filename + "_CR_" + i + ".obj");
		}
		out.format("# Clustering Fibonacci ------\n");
		for (int i = 0; i < count; i++) {
			out.format("#  S[%2$d] : %1$s\n", CF_rS[i].toString(), i);
			out.format("#  Po[%2$d]: %1$s\n", commentOut(CF_rPo[i]), i);
			out.format("#  P[%2$d] : %1$s\n", commentOut(CF_rP[i]), i);
			out.format("# |P[%2$d]|: %1$s\n", CF_rP[i].normSqr(), i);
			out.format("# |L[%2$d]|: %1$s\n", CF_maxLengths[i], i);
			out.format("# |a[%2$d]|: %1$s\n", CF_minAngle[i], i);
			HalfedgeIO.writeOBJ(CF_rS[i], a, FibonacciClusteringG2.filename + "_CF_" + i + ".obj");
		}
		out.format("# Homogeneous Random --------\n");
		for (int i = 0; i < count; i++) {
			out.format("#  S[%2$d] : %1$s\n", HR_rS[i], i);
			out.format("#  Po[%2$d]: %1$s\n", commentOut(HR_rPo[i]), i);
			out.format("#  P[%2$d] : %1$s\n", commentOut(HR_rP[i]), i);
			out.format("# |P[%2$d]|: %1$s\n", HR_rP[i].normSqr(), i);
			out.format("# |L[%2$d]|: %1$s\n", HR_maxLengths[i], i);
			out.format("# |a[%2$d]|: %1$s\n", HR_minAngle[i], i);
			HalfedgeIO.writeOBJ(HR_rS[i], a, FibonacciClusteringG2.filename + "_HR_" + i + ".obj");
		}
		out.format("# Homogeneous Fibonacci -----\n");
		for (int i = 0; i < count; i++) {
			out.format("#  S[%2$d] : %1$s\n", HF_rS[i], i);
			out.format("#  Po[%2$d]: %1$s\n", commentOut(HF_rPo[i]), i);
			out.format("#  P[%2$d] : %1$s\n", commentOut(HF_rP[i]), i);
			out.format("# |P[%2$d]|: %1$s\n", HF_rP[i].normSqr(), i);
			out.format("# |L[%2$d]|: %1$s\n", HF_maxLengths[i], i);
			out.format("# |a[%2$d]|: %1$s\n", HF_minAngle[i], i);
			HalfedgeIO.writeOBJ(HF_rS[i], a, FibonacciClusteringG2.filename + "_HF_" + i + ".obj");
		}
		out.close();
	}

	private static String commentOut(ComplexMatrix P) {
