...
> implements ConformalFunctional<V, E, F> {
		...
		for (final F t : hds.getFaces()) {
			...
			if (G != null) {
				...
				if (var.isVariable(vj)) {
					G.add(var.getVarIndex(vj), -alpha.getAlpha(eki));
				}
				if (var.isVariable(vk)) {
					G.add(var.getVarIndex(vk), -alpha.getAlpha(eij));
				}
			}
		}
	}
	...
	public boolean triangleEnergyAndAlphas(
		...
		final double 
			λij = lambda.getLambda(eij) + ui + uj,
			λjk = lambda.getLambda(ejk) + uj + uk,
			λki = lambda.getLambda(eki) + uk + ui;
		final double
			lij = 2 * arsinh(exp(λij / 2)),
